...
const addTax = (array) => {
	const total = array[1] * 0.0925;
	array.splice(2, 0, total.toFixed(2));
	return array;
};
...
const allTheNumbers = (num) => {
	let array;
	if (num > 0) {
		array = Array.from({length: num}, (v, i) => {return i+1;});
	} else {
		array = Array.from({length: (Math.abs(num) + 2)}, (v, i) => {return 1-i;});
	}
	return array;
};
